...
import {
	isSingleTopicMode,
	pickNextQuestionHelper,
} from "../utils/singleTopicHelper"
...
const GeographyFullPagePicker = ({
	...
	const [mixedQuestions, setMixedQuestions] = useState<FullQuestion[]>([])
	...
	const geo = navState?.geoCount ?? geoCount
	const cult = navState?.cultCount ?? cultCount
	...
	// το Lint με υποχρέωσε να την κάνω callback
	const pickNextQuestion = useCallback(() => {
		if (!singleTopicMode) return

		const geoSource =
			shuffleSingle && shuffledGeo.length ? shuffledGeo : geoQuestions

		const cultSource =
			shuffleSingle && shuffledCulture.length
				? shuffledCulture
				: cultureQuestions

		const histSource =
			shuffleSingle && shuffledHistory.length
				? shuffledHistory
				: historyQuestions

		const instSource =
			shuffleSingle && shuffledInst.length ? shuffledInst : instQuestions
	...
	useEffect(() => {
		...
		pickNextQuestion()
	}, [shuffleSingle, singleTopicMode, pickNextQuestion])
	...
	const pickRandomQuestions = () => {
		...
		const newSelected = {
			geography: shuffledGeo.slice(0, geo),
			culture: shuffledCulture.slice(0, cult),
			history: shuffledHistory.slice(0, hist),
			institutions: shuffledInst.slice(0, inst),
		}

		setSelectedQuestions(newSelected)

		const newAll = [
			...newSelected.geography,
			...newSelected.culture,
			...newSelected.history,
			...newSelected.institutions,
		]

		setMixedQuestions([...newAll].sort(() => 0.5 - Math.random()))
	...
	if (mixedMode) {
		return (
			...
			>
				...
				<Button onClick={() => setMixedMode(false)}>Κατά Θεματική</Button>
	...
	return (
		...
		>
			...
			{singleTopicMode ? (
				...
				<Button onClick={() => setMixedMode(true)}>Ανάμεικτες Ερωτήσεις</Button>
			)}
			...
			{singleTopicMode && (
				<Button onClick={pickNextQuestion}>Επόμενη Ερώτηση</Button>
			)}
