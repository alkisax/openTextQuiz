...
const LanguageTest = ({ test }: LanguageTestProps) => {
	...
	useEffect(() => {
		...
		// desktop
		element.addEventListener("mouseup", highlightSelection)

		// mobile
		element.addEventListener("touchend", highlightSelection)

		return () => {
			element.removeEventListener("mouseup", highlightSelection)
			element.removeEventListener("touchend", highlightSelection)
		}
	}, [])
	...
	return (
		<div className="space-y-10 max-w-5xl mx-auto py-8">
							...
							<div className={`flex-1 p-4 rounded ${gradedClass}`}>
								{q.type === "shortText" && (
									<ShortTextQuestion
										question={q}
										value={
											q.multipleBlanks ? multiAnswers[q.id] : answers[q.id]
										}
										onChange={(value) => {
											if (q.multipleBlanks) {
			...
			<div className="max-w-3xl mx-auto py-8">
				...
				<Button type="button" onClick={gradeEssay} disabled={essayLoading}>
